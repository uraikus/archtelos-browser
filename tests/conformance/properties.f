...
arr[text] func styleDigestFields(s:Style) {
    return [`${s.display}`, `${s.color}`, `${s.background}`, `${s.fontSize}`, 
        ...
        `${anchorInfoOf(s.anchorInfo).name}`, `${anchorInfoOf(s.anchorInfo).anchor}`, `${anchorInfoOf(s.anchorInfo).area}`, `${anchorInfoOf(s.anchorInfo).fallbacks}`, `${anchorInfoOf(s.anchorInfo).tryOrder}`, `${anchorInfoOf(s.anchorInfo).visibility}`, `${anchorInfoOf(s.anchorInfo).scope}`, `${motionKeyPath(motionInfoOf(motionIndexOf(s)))}`, `${lenKey(motionInfoOf(motionIndexOf(s)).distance)}`, `${motionInfoOf(motionIndexOf(s)).rotateMode}|${motionInfoOf(motionIndexOf(s)).rotateAngle}`, `${motionInfoOf(motionIndexOf(s)).anchorAuto ? 1 : 0}|${lenKey(motionInfoOf(motionIndexOf(s)).anchorX)}|${lenKey(motionInfoOf(motionIndexOf(s)).anchorY)}`, `${motionInfoOf(motionIndexOf(s)).posNormal ? 1 : 0}|${lenKey(motionInfoOf(motionIndexOf(s)).posX)}|${lenKey(motionInfoOf(motionIndexOf(s)).posY)}`, `${clipMarginPacked(s)}`, `${textBoxPacked(s)}`, `${decorationIsClone(s) ? 1 : 0}`, `${overscrollPacked(s)}`, `${filterKey(s)}`, `${maskKey(s)}`, `${s.isolate}`, `${s.writingMode}`, `${s.textOrientation}`, `${s.textCombine ? 1 : 0}`, `${fontSynthSmallCapsOf(s) ? 1 : 0}`, `${imageRenderingOf(s)}`, `${willChangeOf(s)}`, `${interactivityInert(s) ? 1 : 0}`, `${scrollInitialTarget(s) ? 1 : 0}`, `${hasViewTransitionName(s) ? 1 : 0}`, 
        ...
        `${s.snapX}`, `${s.snapY}`, `${s.snapStrict}`, `${s.snapAlignBlock}`, `${s.snapAlignInline}`, `${s.snapStopAlways}`, 
        ...
        `${rubyPositionOf(s)}`, `${rubyAlignOf(s)}`, `${printColorAdjustOf(s)}`,
        `${fontCapsOf(s)}`]
}
...
arr[text] func styleDigestFieldNames() {
    return ['display', 'color', 'background', 'fontSize', 'fontBold', 
        ...
        'offsetPath', 'offsetDistance', 'offsetRotate', 'offsetAnchor', 'offsetPosition', 'overflowClipMargin', 'textBox', 'boxDecorationBreak', 'overscrollBehavior', 'filter', 'mask', 'isolation', 'writingMode', 'textOrientation', 'textCombine', 'fontSynthSmallCaps', 'imageRendering', 'willChange', 'interactivity', 'scrollInitialTarget', 'viewTransitionName',
        ...
        'snapX', 'snapY', 'snapStrict', 'snapAlignBlock', 'snapAlignInline', 'snapStopAlways', 
        ...
        'resize', 'textWrapStyle', 'rubyPosition', 'rubyAlign', 'printColorAdjust',
        'fontCaps']
}
