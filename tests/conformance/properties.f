...
arr[text] func styleDigestFields(s:Style) {
    return [`${s.display}`, `${s.color}`, `${s.background}`, `${s.fontSize}`, 
        ...
        `${anchorInfoOf(s.anchorInfo).name}`, `${anchorInfoOf(s.anchorInfo).anchor}`, `${anchorInfoOf(s.anchorInfo).area}`, `${anchorInfoOf(s.anchorInfo).fallbacks}`, 
...
arr[text] func styleDigestFieldNames() {
    return ['display', 'color', 'background', 'fontSize', 'fontBold', 
        ...
        'anchorName', 'positionAnchor', 'positionArea', 'positionTryFallbacks',
